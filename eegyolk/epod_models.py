...
def MLP(X, y, X_train, y_train, X_test, y_test):
    ...
    # model definition
    NN = MLPClassifier(activation=clf.best_params_.get('activation'), alpha=clf.best_params_.get('alpha'),
                       solver=clf.best_params_.get('solver'), learning_rate=clf.best_params_.get('learning_rate'), 
